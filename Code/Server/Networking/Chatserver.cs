...
namespace ChatTCP.Server.Networking
{
    ...
    /// 1. Bind + Listen + Accept liên tục client mới (AcceptLoop).
    /// 2. Cung cấp cơ chế đọc/ghi message có ranh giới rõ ràng (ReadFrame/WriteFrame) để module khác dùng khi đọc/gửi tin nhắn qua TCP stream.
    ...
    public class ChatServer
    {
        ...
        private void AcceptLoop()
        {
            while (_isRunning)
            {
                try
                {
                    ...
                    Console.WriteLine("[ChatServer] Có client mới kết nối: " + ip);
        ...
        /// Đọc đúng 1 message hoàn chỉnh: đọc đủ 4 byte độ dài trước, rồi đọc đủ số byte nội dung mà 4 byte đó khai báo — dù dữ liệu tới làm nhiều đợtvẫn ghép đủ mới trả về. Trả về null nếu client đã ngắt kết nối.
